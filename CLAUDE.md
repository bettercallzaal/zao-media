...
Three paths: (a) locally - steps below; (b) from the web - the "+ Add media" link on the hub opens a GitHub issue form; `.github/workflows/media-submission.yml` converts the issue into a `content/*.json` PR, and merging publishes; (c) the team API - `repository_dispatch` endpoint documented in `docs/api.md` (`.github/workflows/media-api.yml`), JSON in, PR or direct publish out.
